...
let MessageRouter = MessageRouter_1 = class MessageRouter {
    ...
    async route(message, context) {
        ...
        if ([types_1.ConversationState.SCREENING_Q1, types_1.ConversationState.SCREENING_Q2, types_1.ConversationState.SCREENING_Q3].includes(context.state)) {
        ...
        if (selectedState) {
            ...
            if (selectedState === types_1.ConversationState.SCREENING_Q1) {
                return this.screeningHandler.start(updatedContext);
            }
            if (selectedState === types_1.ConversationState.SLEEP_LAB) {
                return this.sleepLabHandler.handle(message, updatedContext);
            }
            if (selectedState === types_1.ConversationState.CPAP) {
                return this.cpapHandler.handle(message, updatedContext);
            }
            if (selectedState === types_1.ConversationState.ELDERLY) {
                return this.elderlyHandler.handle(message, updatedContext);
            }
            if (selectedState === types_1.ConversationState.CONTACT_STAFF) {
        ...
        if (isGreeting || context.state === types_1.ConversationState.START) {
